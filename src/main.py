...
def filter_df(df, column_name, values):
    ...
    :rtype DataFrame:
    """
    log.info(f'filtering on {column_name} with {str(values)}')
...
def rename_columns(df, col_name_mapping):
    """
    return a data frame with the same data, but with columns renamed according to mapping
    :param DataFrame df: the data frame
    :param dict or another mapping col_name_mapping: mapping where keys are existing column names,
...
if __name__ == "__main__":
    ...
    if len(argv) < 4:
        msg = ['please provide 3 space-separated arguments:',
               ...
               'countries to filter. If country names contain space enclose in parentheses',
               'example:',
